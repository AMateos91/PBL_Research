...
from PBL_Research.export import TableExporter, ModelExporter, FigureExporter
...
print(f"Numeric scientific features before coverage filter: {len(feature_names)}")
print("Feature coverage:")
for name, coverage in feature_coverage.items():
    print(f"  {name}: {coverage:.3f}")
feature_names = feature_coverage[feature_coverage >= 0.50].index.tolist()
print(f"Scientific features retained (>=50% coverage): {len(feature_names)}")
...
plt.legend()
timeseries_fig = plt.gcf()
FigureExporter().export(
    timeseries_fig,
    str(OUTPUT / "pbl_height_timeseries.png"),
)
